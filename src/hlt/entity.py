...
class Entity(abc.ABC):
    ...
    def __repr__(self):
        return "{}(id={}, {})".format(self.__class__.__name__, self.id, self.position)
...
class Ship(Entity):
    ...
    def _generate(player_id):
        ...
        return (
            ship_id,
            Ship(player_id, ship_id, Position(x_position, y_position), halite),
        )

    def __repr__(self):
        return "{}(id={}, {}, cargo={} halite)".format(
            self.__class__.__name__, self.id, self.position, self.halite_amount
        )
